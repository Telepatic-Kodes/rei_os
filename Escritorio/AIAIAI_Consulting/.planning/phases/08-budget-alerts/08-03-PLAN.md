...
2. Start dev server, run: `curl -X POST http://localhost:3000/api/settings/alerts -H "Content-Type: application/json" -d '{"globalBudget":200,"globalThresholds":[{"percent":75,"label":"Warning"},{"percent":90,"label":"Critical"}],"perProjectLimits":{},"notificationsEnabled":true,"monthlyResetDay":1}' ` — returns 200
