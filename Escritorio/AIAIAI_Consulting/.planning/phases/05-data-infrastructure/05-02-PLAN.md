...
1. Update each sync script to use shared utilities. For each script:
   ...
   - After appending history, call `cleanupOldHistory(HISTORY_DIR, metric)` to enforce 6-month retention

   For `scripts/sync-tokens.ts` specifically:
   - Import: `import { atomicWriteJson } from "../app/src/lib/atomic-write";`
   - Import: `import { appendJsonl, cleanupOldHistory } from "../app/src/lib/jsonl";`
   ...
   - Then cleanup: `cleanupOldHistory(HISTORY_DIR, "tokens")`
   - Where `HISTORY_DIR = join(ROOT, "data", "history")`
   - Remove the local `TokenEntry` and `TokenData` interfaces
   - Existing dedup logic (session-based filtering) already ensures idempotency

   For `scripts/sync-quality.ts`:
   - Apply same import pattern (atomicWriteJson, appendJsonl, cleanupOldHistory, schemas)
   - Existing dedup logic (date+project key) already ensures idempotency
   - After write + history append, call `cleanupOldHistory(HISTORY_DIR, "quality")`

   For `scripts/sync-projects.ts`:
   - Apply same import pattern (atomicWriteJson, appendJsonl, cleanupOldHistory, schemas)
   - IMPORTANT — explicitly add dedup by project name: Before writing, deduplicate the projects array by `name` field. Use a Map keyed by project name, keeping the latest entry for each name:
     ```typescript
     const deduped = Array.from(
       new Map(projects.map(p => [p.name, p])).values()
     );
     ```
     This ensures running sync-projects.ts twice produces identical output even if the source data contains duplicates or the script re-fetches existing projects.
   - After write + history append, call `cleanupOldHistory(HISTORY_DIR, "projects")`
...
Run: `grep "cleanupOldHistory" scripts/sync-tokens.ts scripts/sync-quality.ts scripts/sync-projects.ts` — all three scripts call cleanup
Run: `grep "new Map" scripts/sync-projects.ts` — confirms dedup by project name
Run: `cat .gitignore` — confirms data/history/ is ignored
  </verify>
  <done>All sync scripts use atomic writes, append to JSONL history, and run retention cleanup. sync-projects.ts deduplicates by project name. No duplicated interfaces remain in scripts. Gitignore properly separates auto-generated from tracked data. Running any sync script twice produces identical files.</done>
</task>
...
7. `grep "cleanupOldHistory" scripts/sync-*.ts` — all three scripts call cleanup
8. `grep "new Map" scripts/sync-projects.ts` — confirms project name dedup
</verification>
...
- All sync scripts call cleanupOldHistory to enforce 6-month retention
- sync-projects.ts deduplicates by project name (Map keyed by name)
- .gitignore ignores data/history/ but tracks data/*.json and data/config/
- `npm run build` succeeds (dashboard works)
- Sync scripts are idempotent (dedup logic confirmed for all three)
</success_criteria>
