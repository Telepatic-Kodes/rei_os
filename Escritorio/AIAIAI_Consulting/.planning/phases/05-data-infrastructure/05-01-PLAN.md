...
must_haves:
  truths:
    ...
    - "JSONL history files older than 6 months are cleaned up by cleanupOldHistory"
  ...
  artifacts:
    ...
    - path: "app/src/lib/jsonl.ts"
      provides: "JSONL read/append/rotate/cleanup utilities with monthly file naming"
      exports: ["appendJsonl", "readJsonl", "getMonthlyPath", "listHistoryFiles", "cleanupOldHistory"]
...
<objective>
Create the foundational data utilities: Zod schemas for all data shapes, atomic write functions (temp file + rename), and JSONL history infrastructure with monthly rotation and retention cleanup.
...
<task type="auto">
  <name>Task 2: JSONL history utilities with retention cleanup and budget config extraction</name>
...
1. Create `app/src/lib/jsonl.ts`:
   ...
   - `cleanupOldHistory(baseDir: string, metric: string, keepMonths: number = 6): string[]` — lists JSONL files for the metric via listHistoryFiles, parses the YYYY-MM from each filename, computes the cutoff date (current month minus keepMonths), deletes files older than the cutoff using fs.unlinkSync, returns array of deleted file paths. Example: if keepMonths=6 and current month is 2026-01, files before 2026-07-* (i.e., 2025-07 and earlier) are deleted. Use a simple string comparison on the YYYY-MM portion since ISO date strings sort lexicographically.
...
Verify cleanupOldHistory is exported: `grep "cleanupOldHistory" app/src/lib/jsonl.ts`
  </verify>
  <done>JSONL utilities create monthly-rotated files with retention cleanup (default 6 months). History directories exist. Budget config extracted to data/config/budget.json. TypeScript compiles.</done>
</task>
...
5. `grep "cleanupOldHistory" app/src/lib/jsonl.ts` — confirms cleanup function exists
</verification>
...
- cleanupOldHistory deletes files older than keepMonths (default 6)
